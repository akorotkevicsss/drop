...
export default function UserProfileScreen() {
  ...
  const cacheKey =
    cleanUsername
      ? `user-profile:${cleanUsername.toLowerCase()}`
      : '';

  const cached =
    cacheKey
      ? getScreenCache<UserProfileCache>(
          cacheKey
        )
      : null;
  ...
  const saveCache =
    (
      ...
      if (cacheKey) {
        setScreenCache<UserProfileCache>(
          cacheKey,
          value
        );
      }
          ...
          (
            ...
            (row) => {
              nextJoinStatuses[
                ...
                row.status as JoinStatus;
            }
          );

          (
            myRatingsResult.data ??
            []
          ).forEach(
            (row) => {
              nextMyRatings[
  ...
  useEffect(() => {
    ...
    if (nextCached) {
      ...
      setUserDrops(
        nextCached.userDrops
      );

      setCurrentUserId(
        nextCached.currentUserId
      );

      setAverageEventRate(
        nextCached.averageEventRate
      );

      setEventRatingsCount(
        nextCached.eventRatingsCount
      );

      setDropAverageRatings(
        nextCached.dropAverageRatings
      );

      setJoinStatuses(
        nextCached.joinStatuses
      );

      setMyRatings(
        nextCached.myRatings
      );
    ...
    async () => {
      ...
      try {
        ...
        if (
          ...
          Alert.alert(
            'Error',
            'Could not send this message request.'
          );
          return;
        }
      ...
      >
        ...
        {averageEventRate !==
          ...
          >
            ...
            <Text
              style={
                styles.eventRateValue
              }
            >
              ★{' '}
              {averageEventRate.toFixed(
                1
              )}
            </Text>
          </View>
