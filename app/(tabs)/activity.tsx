...
function getActionText(type: NotificationType) {
  switch (type) {
    case 'like': return 'liked your Drop';
    case 'join_request': return 'wants to join your Drop';
    case 'join_accepted': return 'accepted your Join request';
    case 'reply': return 'replied to your Drop';
...
export default function ActivityScreen() {
  ...
  const markAllRead = useCallback(async (userId: string) => {
    const { error } = await supabase
      ...
      .eq('user_id', userId)
      .is('read_at', null);
    if (error) console.error('MARK ACTIVITY READ ERROR:', error);
  }, []);
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('notifications')
        .select(`id,user_id,actor_id,type,drop_id,conversation_id,message_id,text_snapshot,read_at,created_at`)
        .eq('user_id', user.id)
      ...
      const raw = data ?? [];
      const actorIds = [...new Set(raw.map((item) => item.actor_id).filter((v): v is string => !!v))];
      ...
      if (actorIds.length > 0) {
        const { data: profileData, error: profileError } = await supabase
          ...
          .in('id', actorIds);
        if (profileError) console.error('ACTIVITY PROFILES ERROR:', profileError);
  ...
  const findUnifiedConversation = async (actorId: string) => {
    if (!currentUserId) return null;
    const { data, error } = await supabase
      .from('conversations')
      .select('id')
      .or(`and(author_id.eq.${currentUserId},participant_id.eq.${actorId}),and(author_id.eq.${actorId},participant_id.eq.${currentUserId})`)
      .limit(1)
      .maybeSingle();
    if (error) {
  ...
  const openNotification = async (notification: NotificationRow) => {
    if (notification.type === 'join_request' && notification.drop_id) {
      router.push({ pathname: '/requests', params: { dropId: notification.drop_id } });
      return;
    }
    ...
    if (notification.actor_id) {
      const conversationId = await findUnifiedConversation(notification.actor_id);
      if (conversationId) {
        router.push(`/chat/${conversationId}`);
        return;
      }
    }
    Alert.alert('Activity', 'This item is no longer available.');
  };
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerText}>
          ...
          <Text style={styles.subtitle}>
            See what’s happening around your Drops.
          </Text>
        </View>
      </View>
        ...
        <ScrollView>
          {notifications.map((notification) => {
            const actorName =
              ...
              'Someone';
            const action = getActionText(notification.type);
                ...
                <View style={styles.notificationContent}>
                  ...
                  {!!notification.text_snapshot && (
                    <Text
                      ...
                      numberOfLines={notification.type === 'message' ? 1 : 2}
                    >
                      {notification.text_snapshot}
                    </Text>
                  )}
